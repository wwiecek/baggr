...
Therefore, the result we get from the two models will be close (albeit with some numerical variation due to short run time):

```{r}
baggr_compare(bg_model_agg, bg_model_ind)
```

There will be difference in speed -- in our example logistic model has to work with `r nrow(df_ind)` rows of data, while the summary level model only had 7 datapoints. Therefore in all standard cases you are better off summarising your data and using the fastest model.
...
* If your data includes covariates or events are rare, you should consider the logistic model. We will show why below.
