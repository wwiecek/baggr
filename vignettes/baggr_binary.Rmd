...
df <- prepare_ma(df_yusuf, group = "trial", effect = "logOR")
df
```
...
```{r, include = F}
bg_model_agg <- baggr(df, iter = 5000, effect = "logarithm of odds ratio")
```

```{r, eval = F, echo = T}
bg_model_agg <- baggr(df, effect = "logarithm of odds ratio")
```
...
```{r}
labbe(df, plot_model = TRUE, shade_se = "or")
```
...
bg_model_ind <- baggr(df_ind, model = "logit", effect = "logarithm of odds ratio", iter = 5000)
```
