...
For the first example we will use a simple summary data based on (part of) Table 6 in @yusuf_beta_1985, a famous study of impact of beta blockers on occurence of strokes and mortality.
...
In Study 1 you can see that no events occured in `a` column. Assume, that we have individual-level data for these studies (if not, we can generate it as we did in the example above):
