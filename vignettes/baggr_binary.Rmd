...
# Basic binary data meta-analysis of odd ratios

Typically, a meta-analysis of binary data is done on summary statistics such as $\log(OR)$ or $\log(RR)$. The reason for this is two-fold: 1) they are most commonly reported statistics and 2) they are approximately normally distributed. The second assumption needs to be treated carefully, as we will show later.


We start from a simple summary data based on Table 6 in @yusuf_beta_1985
...
In a typical notation, `a` (`c`) are numbers of events in treatment (control) groups, while `n1` (`n2`) are total patients in treatment (control) group:



$\log(OR)$ and its SE can easily be calculated from the values (if you're not faimilar with OR, [see here](https://en.wikipedia.org/wiki/Odds_ratio#Statistical_inference)):

```{r}
df <- df_yusuf %>%
  mutate(b=n1i-a, d = n2i-c) %>% #we use dplyr for data manipulation
  mutate(tau = log((a*d)/(b*c)),
         se  = sqrt(1/a + 1/b + 1/c + 1/d))
df
```

We use `tau` and `se` notation, same as we would for continuous data with `baggr()`. In fact, the model we use for logOR is the same default "Rubin" model (@rubin_estimating_1974) with partial pooling, same as for any continuous quantities in _baggr_. 
...
As with continuous quantities, the prior is chosen automatically. However, it is advisable to review the automatic prior choice when analysing logged quantities, because the priors may be needlessly diffuse.
...
gridExtra::grid.arrange(
  plot(bg_model_agg, transform = exp),
  effect_plot(bg_model_agg, transform = exp) + xlim(0, 3),
  ncol = 2)
```
...
Let's assume that you have access to underlying individual-level data. In our example above, we do not, but we can generate the numbers as we know the contingency table:
...
Note: as in other cases, `baggr()` will detect model appropriately (in this case by noting that `outcome` has only 0's and 1's) and notify user, so in everyday use setting `model = "logit"` is not necessary.
...
This means, the modelling assumption is the same as when working with summarised data. Because baggr's default prior is set in the same way for summary and individual-level data (you can see it as `bg_model_ind$formatted_prior`), the result we get is the same (with some numerical variation due to short run time):

```{r}
baggr_compare(bg_model_agg, bg_model_ind)
```

The only difference is in speed -- logistic model has to work with `r nrow(df_ind)` rows of data, while the summary level model only has 7 datapoints. Therefore you are better off summarising your data and using the default model, which I will show next.
...
As shown above, you can create data for logistic model from `a`, `b`/`n1`, `c`, `d`/`n2` columns. You cannot do it from OR only, as the denominator is missing.
