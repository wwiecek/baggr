...
auto_prior <- function(data, stan_data, model, outcome = "outcome",
  ...
  if(model %in% c("rubin")) {
    prior_list[["prior_upper_sigma_tau"]] <- 5*var(data$tau)
  ...
  if(model == "mutau") {
    ...
    prior_list[["prior_upper_sigma_tau"]] <- c(5*var(data$mu), 5*var(data$tau))
    message(paste0("* sigma_mu ~ Uniform(0, ",
                   round(prior_list[["prior_upper_sigma_tau"]][1], 2), ")"))
    message(paste0("* sigma_tau ~ Uniform(0, ",
                   round(prior_list[["prior_upper_sigma_tau"]][2], 2), ")"))
  ...
  if(model == "full") {
    ...
    prior_list[["mutau_prior_sigma"]] <- 5*vhat*diag(prior_list$P)
  }
  if(model == "quantiles") {
